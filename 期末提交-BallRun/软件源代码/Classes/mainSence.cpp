...
bool mainSence::init()
{
	...
	auto origin = Director::getInstance()->getVisibleOrigin();
	 ...
	 m_timer = GameTimer::createTimer(30);
...
void mainSence::tick(float dt)
{
	if (succe == 0){
		...
		char* mtime = new char[40];
		int pTime = 300 - (int)m_timer->gettime();
		int number = (int)m_timer->gettime() / 3;
	
			int score = UserDefault::sharedUserDefault()->getIntegerForKey("maxscore");
			if (score == 0)
			{
				UserDefault::sharedUserDefault()->setIntegerForKey("maxscore", number);
			}
			if (number > (UserDefault::sharedUserDefault()->getIntegerForKey("maxscore")))
			{
				sprintf(mtime, "WIN!!LEF:%d : %d---SCORE:%d  maxscore:%d", (int)m_timer->gettime() / 60, (int)m_timer->gettime() % 60, (int)number, (int)number);
				log(mtime);
			...
			{
				sprintf(mtime, "WIN!!LEF:%d : %d---SCORE:%d  maxscore:%d", (int)m_timer->gettime() / 60, (int)m_timer->gettime() % 60, (int)number, (int)(UserDefault::sharedUserDefault()->getIntegerForKey("maxscore")));
				log(mtime);
				gameOverScene->getLayer()->getLabel()->setString(mtime);
				Director::getInstance()->replaceScene(gameOverScene);
			}
...
void mainSence::btnBacked(Ref *pSender, Widget::TouchEventType type)
{
	...
	{
		log("touch ended");
		Director::getInstance()->replaceScene(Test::createScene());
	}
}
...
curball_point = ball->getPosition();
	...
	int number = (int)m_timer->gettime() / 3;
	if (number < 1){
		auto mgameOverScene = GameOverScene::create();
		char* mmtime = new char[40];
		sprintf(mmtime, "FAIL!!");
		log(mmtime);
		mgameOverScene->getLayer()->getLabel()->setString(mmtime);
		Director::getInstance()->replaceScene(mgameOverScene);
	}
